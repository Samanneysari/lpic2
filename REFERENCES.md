# Official References

## Certification

- [LPI LPIC-2 overview](https://www.lpi.org/our-certifications/lpic-2-overview/)
- [LPI exam 201 and 202 objectives](https://www.lpi.org/our-certifications/exam-201-202-objectives/)
- [LPI Learning Materials](https://learning.lpi.org/)

## Core Linux

- [Linux kernel documentation](https://docs.kernel.org/)
- [systemd manual](https://www.freedesktop.org/software/systemd/man/)
- [GNU GRUB manual](https://www.gnu.org/software/grub/manual/grub/)
- [Linux man-pages](https://www.kernel.org/doc/man-pages/)
- [Btrfs documentation](https://btrfs.readthedocs.io/)
- [XFS documentation](https://docs.kernel.org/filesystems/xfs.html)

## Network services

- [BIND 9 Administrator Reference Manual](https://bind9.readthedocs.io/)
- [ISC DHCP manual archive](https://kb.isc.org/docs/isc-dhcp-44-manual-pages-dhcpdconf)
- [Apache HTTP Server 2.4](https://httpd.apache.org/docs/2.4/)
- [Nginx documentation](https://nginx.org/en/docs/)
- [Squid configuration reference](https://www.squid-cache.org/Doc/config/)
- [HAProxy configuration manual](https://www.haproxy.com/documentation/haproxy-configuration-manual/latest/)
- [PHP-FPM documentation](https://www.php.net/manual/en/install.fpm.php)
- [Certbot user guide](https://eff-certbot.readthedocs.io/en/stable/using.html)
- [Samba documentation](https://www.samba.org/samba/docs/)
- [OpenLDAP Administrator Guide](https://www.openldap.org/doc/admin24/)
- [Postfix documentation](https://www.postfix.org/documentation.html)
- [Dovecot documentation](https://doc.dovecot.org/)
- [OpenSSH manuals](https://www.openssh.com/manual.html)
- [OpenVPN community resources](https://openvpn.net/community-resources/)

## Protocols

- [DNS concepts: RFC 1034](https://www.rfc-editor.org/rfc/rfc1034)
- [DNS implementation: RFC 1035](https://www.rfc-editor.org/rfc/rfc1035)
- [Common DNS operational errors and forward/reverse consistency: RFC 1912](https://www.rfc-editor.org/rfc/rfc1912)
- [Classless IPv4 reverse delegation: RFC 2317](https://www.rfc-editor.org/rfc/rfc2317)
- [IPv6 DNS extensions and AAAA: RFC 3596](https://www.rfc-editor.org/rfc/rfc3596)
- [Current DNS terminology: RFC 9499](https://www.rfc-editor.org/rfc/rfc9499)
- [HTTP specifications maintained by the IETF HTTP Working Group](https://httpwg.org/specs/)
- [TLS 1.3: RFC 8446](https://www.rfc-editor.org/rfc/rfc8446)
- [DHCP: RFC 2131](https://www.rfc-editor.org/rfc/rfc2131)
- [BOOTP: RFC 951](https://www.rfc-editor.org/rfc/rfc951)
- [Private IPv4 addresses: RFC 1918](https://www.rfc-editor.org/rfc/rfc1918)
- [IPv4 documentation addresses: RFC 5737](https://www.rfc-editor.org/rfc/rfc5737)
- [IPv6 documentation prefix: RFC 3849](https://www.rfc-editor.org/rfc/rfc3849)
- [mDNS and the special .local domain: RFC 6762](https://www.rfc-editor.org/rfc/rfc6762)

## Security

- [Red Hat SELinux guide](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/using_selinux/)
- [Debian security](https://www.debian.org/security/)
- [Ubuntu security notices](https://ubuntu.com/security/notices)
- [CISA Known Exploited Vulnerabilities](https://www.cisa.gov/known-exploited-vulnerabilities-catalog)

The objective version includes older tools. Learn the named exam terms, but check current production guidance before deployment.

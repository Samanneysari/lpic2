# LPIC-2 Objective Coverage Checklist

This checklist follows LPIC-2 objectives version 4.5. Mark an item only after you can explain it and complete a small lab without copying the answer.

## Exam 201-450

- [ ] 200.1 Measure and troubleshoot resource usage — weight 6
- [ ] 200.2 Predict future resource needs — weight 2
- [ ] 201.1 Kernel components — weight 2
- [ ] 201.2 Compiling a Linux kernel — weight 3
- [ ] 201.3 Kernel runtime management and troubleshooting — weight 4
- [ ] 202.1 Customizing system startup — weight 3
- [ ] 202.2 System recovery — weight 4
- [ ] 202.3 Alternate bootloaders — weight 2
- [ ] 203.1 Operating the Linux filesystem — weight 4
- [ ] 203.2 Maintaining a Linux filesystem — weight 3
- [ ] 203.3 Creating and configuring filesystem options — weight 2
- [ ] 204.1 Configuring RAID — weight 3
- [ ] 204.2 Adjusting storage device access — weight 2
- [ ] 204.3 Logical Volume Manager — weight 3
- [ ] 205.1 Basic networking configuration — weight 3
- [ ] 205.2 Advanced network configuration — weight 4
- [ ] 205.3 Troubleshooting network issues — weight 4
- [ ] 206.1 Make and install programs from source — weight 2
- [ ] 206.2 Backup operations — weight 3
- [ ] 206.3 Notify users about system-related issues — weight 1

## Exam 202-450

- [ ] 207.1 Basic DNS server configuration, hierarchy, resolution lifecycle, and resource records — weight 3
- [ ] 207.2 Create and maintain forward and reverse zones, including rDNS/PTR — weight 3
- [ ] 207.3 Secure a DNS server with role separation, TSIG, DNSSEC, and controlled transfers — weight 2
- [ ] 208.1 Basic Apache configuration — weight 4
- [ ] 208.2 Apache configuration for HTTPS — weight 3
- [ ] 208.3 Implement Squid as a caching proxy — weight 2
- [ ] 208.4 Implement Nginx as a web server and reverse proxy — weight 2
- [ ] 209.1 Samba server configuration — weight 5
- [ ] 209.2 NFS server configuration — weight 3
- [ ] 210.1 DHCP configuration — weight 2
- [ ] 210.2 PAM authentication — weight 3
- [ ] 210.3 LDAP client usage — weight 2
- [ ] 210.4 Configure an OpenLDAP server — weight 4
- [ ] 211.1 Use email servers — weight 4
- [ ] 211.2 Manage email delivery — weight 2
- [ ] 211.3 Manage mailbox access — weight 2
- [ ] 212.1 Configure a router — weight 3
- [ ] 212.2 Manage FTP servers — weight 2
- [ ] 212.3 Secure shell — weight 4
- [ ] 212.4 Security tasks — weight 3
- [ ] 212.5 OpenVPN — weight 2

## Readiness test

For each checked objective, you should be able to:

1. Define the important terms.
2. Name the main files and commands.
3. Read and correct a small configuration.
4. Validate a change before applying it.
5. Find the relevant logs.
6. Explain one common failure and its fix.
7. Complete the matching lab.
